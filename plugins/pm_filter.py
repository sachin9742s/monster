...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        buttons.append(
            [InlineKeyboardButton(text=f"🎶 Pαɠҽʂ 1/{data['total']}",callback_data="Pαɠҽʂ 🎶")]
        )
        poster=None
        if API_KEY:
            poster=await get_poster(search)
        if poster:
            await message.reply_photo(photo=poster, caption=f"<b>🎬𝐅𝐢𝐥𝐦/𝐒𝐞𝐫𝐢𝐞𝐬 ☛ {search}\n🕐 𝐓𝐢𝐦𝐞 : <b>#DURATION</b>\n📥 𝐌𝐨𝐯𝐢𝐞 𝐑𝐚𝐭𝐢𝐧𝐠 : ⭐️⭐️<b>#RATING</b>  | <a href="#IMDB_URL">IMDb</a>\n \n \n© By:- [Kiccha Request](https://t.me/KicchaRequest) ‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
        else:
            await message.reply_text(f"<b>🎬𝙔𝙤𝙪𝙧 𝙈𝙤𝙫𝙞𝙚 ☞ {search} ‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
...
async def group(client, message):
    ...
    if 2 < len(message.text) < 50:    
        ...
        if len(btn) > 5: 
            ...
            buttons = btn
            buttons.append(
                [InlineKeyboardButton(text="🎶 Pαɠҽʂ 1/1",callback_data="Pαɠҽʂ 🎶")]
            )
            poster=None
            if API_KEY:
                poster=await get_poster(search)
            if poster:
                await message.reply_photo(photo=poster, caption=f"<b>🎬𝐅𝐢𝐥𝐦/𝐒𝐞𝐫𝐢𝐞𝐬 ☛ {search}\n🕐 𝐓𝐢𝐦𝐞 : <b>#DURATION</b>\n📥 𝐌𝐨𝐯𝐢𝐞 𝐑𝐚𝐭𝐢𝐧𝐠 : ⭐️<b>#RATING</b>  | <a href="#IMDB_URL">IMDb</a>\n \n \n© By:- [Kiccha Request](https://t.me/KicchaRequest) ‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            else:
                await message.reply_text(f"<b>🎬𝙔𝙤𝙪𝙧 𝙈𝙤𝙫𝙞𝙚 ☛ {search} ‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            return
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            if int(index) == int(data["total"]) - 2:
                ...
                buttons.append(
                    [InlineKeyboardButton("⌫ 𝑪𝒐𝒎𝒆 𝑩𝒂𝒄𝒌 ⌧", callback_data=f"back_{int(index)+1}_{keyword}")]
                )
                buttons.append(
                    [InlineKeyboardButton(f"🎶 Pαɠҽʂ {int(index)+2}/{data['total']}", callback_data="Pαɠҽʂ 🎶")]
                )
                ...
                buttons.append(
                    [InlineKeyboardButton("⌫ 𝑪𝒐𝒎𝒆 𝑩𝒂𝒄𝒌 ⌧", callback_data=f"back_{int(index)+1}_{keyword}"),InlineKeyboardButton("⋙ 𝑵𝒆𝒙𝒕 𝑷𝒂𝒈𝒆 ⋙", callback_data=f"next_{int(index)+1}_{keyword}")]
                )
                buttons.append(
                    [InlineKeyboardButton(f"🎶 Pαɠҽʂ {int(index)+2}/{data['total']}", callback_data="Pαɠҽʂ 🎶")]
                )
            ...
            if int(index) == 1:
                ...
                buttons.append(
                    [InlineKeyboardButton(f"🎶 Pαɠҽʂ {int(index)}/{data['total']}", callback_data="Pαɠҽʂ 🎶")]
                )
